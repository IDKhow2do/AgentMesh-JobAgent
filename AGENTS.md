...
- For a new round, `jobagent round start` first returns the shared AgentMesh360 `interaction_required` target-role confirmation. Render a native host prompt card when supported; otherwise relay `interaction.fallback_text`. Do not create or imply a round until the answer is continued with `--accept-suggested` and/or explicit `--target-role` values. If the user already named the role, pass it directly and do not ask again.

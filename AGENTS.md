...
- Zhilian may keep a search page in `loading` or briefly expose conflicting login/account controls during a slow navigation. Treat `zhilian_session_state_unknown` and `zhilian_page_state_unknown` as inconclusive, not logged out; follow their exact recovery and do not ask for another login unless the CLI returns `zhilian_login_required`. Never hard-code a replacement `jl` city code. The client learns a changed code only from independent readable page evidence and fails closed with no charge when the requested city cannot be verified.

...
This module keeps recurring job-search work cheap and reviewable:
- stable cross-run job fingerprints and first/last-seen metadata
- profile-aware decision cache so unchanged jobs do not need to be re-reviewed
- a code-enforced final-review authorization for recruiting-platform delivery
- a human-readable Markdown review artifact for the user's final approval

All state is expected to live below .jobagent-local/ or career/private/ and must
never be committed.
"""
...
def job_fingerprint(job: dict[str, Any]) -> str:
    """Fingerprint material changes while ignoring transient page metadata."""
    material = {
...
def profile_digest(career_dir: Path) -> str:
    """Invalidate cached decisions when the user's career facts/preferences change."""
    names = (
        "MASTER_PROFILE.md",
        "PROFILE.md",
        "TARGETS.md",
...
def annotate_job_history(
    jobs: list[dict[str, Any]],
    index_path: Path,
) -> dict[str, int]:
    """Annotate jobs as new/changed/seen and update the persistent index."""
...
def authorize_platform_review(payload: dict[str, Any], approved_keys: list[str] | None = None) -> dict[str, Any]:
    ...
    keys = list(dict.fromkeys(approved_keys or current_keys))
...
def authorized_selected_jobs(payload: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    return [
        job
        for job in _selected_jobs(payload)
        if str(job.get("canonical_job_key") or canonical_job_key(job)) in keys
    ]
